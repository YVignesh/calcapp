...
class _ElectricityScreenState extends State<ElectricityScreen> {
  final List<_Appliance> _appliances = [
    _Appliance(name: TextEditingController(text: 'LED Bulb'),
  ...
  void _calculate() {
    ...
    setState(() {
      _daily = '\$${_fmt.format(totalKwh * rate)} / ${totalKwh.toStringAsFixed(3)} kWh';
  ...
  Widget build(BuildContext context) {
    ...
    return CalcScaffold(
      ...
      description: 'Estimate the electricity cost of any appliance based on wattage, daily usage hours, and your local rate per kWh.',
      child: Column(
        ...
        children: [
                ...
                child: Column(children: [
                  Row(children: [
                    Expanded(child: TextField(
                      controller: a.name,
                      decoration: const InputDecoration(hintText: 'Appliance name'),
                    )),
                  ...
                  Row(children: [
                    Expanded(child: TextField(
                      controller: a.watts,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: const InputDecoration(hintText: 'Watts', suffixText: 'W'),
                    )),
                    const SizedBox(width: 8),
                    Expanded(child: TextField(
                      controller: a.hours,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: const InputDecoration(hintText: 'Hours/day', suffixText: 'h'),
                    )),
                    const SizedBox(width: 8),
                    Expanded(child: TextField(
          ...
          TextButton.icon(
            onPressed: () => setState(() => _appliances.add(_Appliance(
  ...
  @override
  void dispose() {
    for (final a in _appliances) { a.dispose(); }
    _rate.dispose();
    super.dispose();
  }
}
...
class _Appliance {
  ...
  _Appliance({required this.name, required this.watts, required this.hours, required this.quantity});
  void dispose() { name.dispose(); watts.dispose(); hours.dispose(); quantity.dispose(); }
}
